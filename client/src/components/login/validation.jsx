...
const validation = (userData) =>{
    ...
    if(!userData.nombre){
        errors.nombre= <span style={{ color: 'red' }}>Debe ingresar un nombre</span>
    }
    if (userData.nombre.length > 30) {
        errors.nombre = <span style={{ color: 'red' }}>El nombre no debe tener más de 30 caracteres</span>
    };
    ...
    if(!userData.email){
        errors.email= <span style={{ color: 'red' }}>Debe ingresar un email</span>
    }
    if(userData.email.length > 35){
        errors.email=<span style={{ color: 'red' }}>máximo 35 caracteres</span>
    };

    if(!/.*\d+.*/.test(userData.password)){ // .test es un metodo de regex que valida si algo se cumple
        errors.password=<span style={{ color: 'red' }}>password tener al menos un número</span>
    }
